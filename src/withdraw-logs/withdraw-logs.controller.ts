...
import {WithdrawLogsResponse} from './interfaces/withdraw-logs.interface';
...
export class WithdrawsLogsController {
  ...
  findAll(@Query() query): Promise<WithdrawLogsResponse> {
    const { page = 1, size = 10, withdrawId, event } = query;
    return this.withdrawLogService.findAll(Number(page), Number(size), withdrawId, event);
  }
}
